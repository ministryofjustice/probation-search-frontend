...
| `resultPath` | Optionally override the `resultPath` configured in the `CaseSearchService`. `:crn` will be replaced at render time with the case reference number (CRN).                                   | "/case/:crn"                                                                                 |
...
| `extraColumns`    | An optional array of extra columns to display in search results, in addition to Name, CRN, and Date of Birth. For example, to add an Age column: `[{ header: 'Age', result => ({ text: result.age }) }]`. | []                                        |
